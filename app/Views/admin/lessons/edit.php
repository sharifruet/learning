<div class="fade-in-up">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0">Edit Lesson: <?= esc($lesson['title']) ?></h2>
        <a href="<?= base_url('admin/modules/' . $lesson['module_id'] . '/edit') ?>" class="btn btn-secondary">
            <i class="bi bi-arrow-left me-2"></i>Back to Module
        </a>
    </div>

    <form method="POST" action="<?= base_url('admin/lessons/' . $lesson['id'] . '/update') ?>" id="lessonForm">
        <?= csrf_field() ?>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select" id="status" name="status">
                            ...
                            <option value="published" <?= old('status', $lesson['status'] ?? 'draft') === 'published' ? 'selected' : '' ?>>Published</option>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="content_type" class="form-label">Content Type</label>
                        <select class="form-select" id="content_type" name="content_type">
                            <option value="text" <?= old('content_type', $lesson['content_type'] ?? 'text') === 'text' ? 'selected' : '' ?>>Text</option>
                            <option value="video" <?= old('content_type', $lesson['content_type'] ?? 'text') === 'video' ? 'selected' : '' ?>>Video</option>
                            <option value="mixed" <?= old('content_type', $lesson['content_type'] ?? 'text') === 'mixed' ? 'selected' : '' ?>>Mixed</option>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="estimated_time" class="form-label">Estimated Time (minutes)</label>
                        <input type="number" class="form-control" id="estimated_time" name="estimated_time" value="<?= old('estimated_time', $lesson['estimated_time'] ?? '') ?>" min="1">
                    </div>
                </div>
                <div class="mb-3">
                    <label for="objectives" class="form-label">Learning Objectives</label>
                    <textarea class="form-control" id="objectives" name="objectives" rows="3"><?= old('objectives', $lesson['objectives'] ?? '') ?></textarea>
                    <small class="form-text text-muted">What students will learn in this lesson (one per line)</small>
                </div>
                <div class="mb-3">
                    <label for="featured_image" class="form-label">Featured Image URL</label>
                    <div class="input-group">
                        <input type="text" class="form-control" id="featured_image" name="featured_image" value="<?= old('featured_image', $lesson['featured_image'] ?? '') ?>" placeholder="Image URL">
                    ...
                    <?php if (!empty($lesson['featured_image'])): ?>
                        <div class="mt-2">
                            <img src="<?= esc($lesson['featured_image']) ?>" alt="Featured" class="img-thumbnail" style="max-height: 150px;">
                        </div>
                    <?php endif; ?>
                </div>
                <div class="mb-3">
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <div id="content" style="min-height: 400px;"><?= old('content', $lesson['content'] ?? '') ?></div>
                    <textarea name="content" id="content_hidden" style="display: none;"><?= old('content', $lesson['content'] ?? '') ?></textarea>
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <textarea class="form-control font-monospace" id="code_examples" name="code_examples" rows="10" style="font-family: 'Courier New', monospace;"><?= old('code_examples', $lesson['code_examples'] ?? '') ?></textarea>
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <a href="<?= base_url('admin/exercises/' . $lesson['id']) ?>" class="btn btn-outline-info">
                    <i class="bi bi-pencil-square me-2"></i>Manage Exercises
                </a>
            </div>
            <div class="d-flex gap-2">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle me-2"></i>Update Lesson
                </button>
            </div>
        </div>
    </form>
