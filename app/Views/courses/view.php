...
<!-- Course Content Wrapper with proper spacing -->
<div class="course-view-wrapper" style="padding-bottom: 6rem; margin-bottom: 2rem;">
    <div class="row">
        <!-- Main Content Column -->
        <div class="col-lg-8">
            <!-- Course Header Card -->
            <div class="card shadow-lg mb-4">
                <div class="card-body p-4">
                    ...
                    <!-- Enrollment Button (hidden for parent courses) -->
                    <?php if (!isset($isParentCourse) || !$isParentCourse): ?>
                        <div class="mt-4">
                            <?php if (session()->has('user_id')): ?>
                                ...
                                <?php elseif ($canAccess): ?>
                                    <!-- Free course - can access without enrollment -->
                                    <?php if (!empty($course['modules'])): ?>
                                        <?php if ($course['enrollment_type'] === 'open'): ?>
                                            <a href="<?= base_url('courses/' . $course['slug'] . '/module/' . $course['modules'][0]['id']) ?>" class="btn btn-primary btn-lg me-2">
                                            ...
                                            <form method="POST" action="<?= base_url('enroll/' . $course['slug']) ?>" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-outline-primary btn-lg">
                                                    <i class="bi bi-plus-circle me-2"></i>Enroll (Optional)
                                                </button>
                                            </form>
                                        <?php else: ?>
                                            <a href="<?= base_url('courses/' . $course['slug'] . '/module/' . $course['modules'][0]['id']) ?>" class="btn btn-primary btn-lg">
                                                <i class="bi bi-play-circle me-2"></i>Start Learning
                                            </a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <?php if ($canAccess && !empty($course['modules'])): ?>
                                    <!-- Free course - can access without login -->
                                    <a href="<?= base_url('courses/' . $course['slug'] . '/module/' . $course['modules'][0]['id']) ?>" class="btn btn-primary btn-lg me-2">
                                        <i class="bi bi-play-circle me-2"></i>Start Learning
                                    </a>
                                    <a href="<?= base_url('auth/login') ?>" class="btn btn-outline-primary btn-lg">
                                        <i class="bi bi-box-arrow-in-right me-2"></i>Log In to Track Progress
                                    </a>
                                <?php else: ?>
            ...
            <!-- Subcourses Section (for parent courses) -->
            <?php if (isset($isParentCourse) && $isParentCourse && !empty($subcourses)): ?>
                <div class="course-subcourses-section" style="margin-bottom: 4rem;">
                    <h3 class="mb-4"><span class="text-gradient">Course Program</span></h3>
                    <p class="lead text-muted mb-4">This comprehensive program consists of <?= count($subcourses) ?> courses. Click on any course below to begin learning.</p>
                    <div class="row g-4">
                        <?php foreach ($subcourses as $index => $subcourse): ?>
                            <div class="col-md-6">
                                <div class="card h-100 shadow-sm border-primary">
                                    <div class="card-body p-4">
                                        <div class="d-flex align-items-center mb-3">
                                            ...
                                            <div class="flex-grow-1">
                                                <h5 class="card-title mb-0"><?= esc($subcourse['title']) ?></h5>
                                                <span class="badge bg-<?= $subcourse['difficulty'] === 'beginner' ? 'success' : ($subcourse['difficulty'] === 'intermediate' ? 'warning' : 'danger') ?> mt-1">
                                                    <?= ucfirst($subcourse['difficulty']) ?>
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <?php endif; ?>
                                        <?php if (!empty($subcourse['modules'])): ?>
                                            <div class="mb-3">
                                                <small class="text-muted">
                                                    <i class="bi bi-list-ul me-1"></i>
                                                    <?= count($subcourse['modules']) ?> module<?= count($subcourse['modules']) !== 1 ? 's' : '' ?>
                                                </small>
                                            </div>
                                        <?php endif; ?>
                                        <a href="<?= base_url('courses/' . $subcourse['slug']) ?>" class="btn btn-primary w-100">
            ...
            <!-- Course Modules Section (for regular courses) -->
            <?php elseif (!empty($course['modules'])): ?>
                <div class="course-modules-section" style="margin-bottom: 4rem;">
            ...
            <?php else: ?>
                <div class="alert alert-info">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="mb-1">No content available</h5>
                            <p class="mb-0">
                                <?php if (isset($isParentCourse) && $isParentCourse): ?>
                                    This program doesn't have any subcourses yet.
                                <?php else: ?>
